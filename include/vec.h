...
class Vec3
{
  ...
  public:
	...
	Vec3(FltPnt x, FltPnt y, FltPnt z) : x_(x), y_(y), z_(z) {}
	...
	{
		return (std::fabs(x_) < 1e-9) && (std::fabs(y_) < 1e-9) && (std::fabs(z_) < 1e-9);
	}
};
...
class Vec2
{
  ...
  public:
	...
	Vec2(FltPnt x, FltPnt y) : x_(x), y_(y) {}
